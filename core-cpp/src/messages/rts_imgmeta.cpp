...
std::vector<std::uint8_t> RtsImgmeta::pack() const {
...
RtsImgmeta RtsImgmeta::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "RTS_IMGMETA body requires " << kBodySize
            << " bytes, got " << length;
