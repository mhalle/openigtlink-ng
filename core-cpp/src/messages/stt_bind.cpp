...
std::vector<std::uint8_t> SttBind::pack() const {
    const std::size_t body_size = (8) + (2) + (type_ids.size() * 12) + (2) + (name_table.size() * 1);
    std::vector<std::uint8_t> out(body_size);
    std::size_t off = 0;
    // resolution
    oigtl::runtime::byte_order::write_be_u64(out.data() + off, resolution);
    off += 8;
    // ncmessages
...
SttBind SttBind::unpack(const std::uint8_t* data, std::size_t length) {
    SttBind out;
    std::size_t off = 0;
    // resolution
    if (off + (8) > length) { throw oigtl::error::ShortBufferError("resolution: short buffer"); }
    out.resolution = oigtl::runtime::byte_order::read_be_u64(data + off);
    off += 8;
    // ncmessages
