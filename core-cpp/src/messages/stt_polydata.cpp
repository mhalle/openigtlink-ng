...
std::vector<std::uint8_t> SttPolydata::pack() const {
...
SttPolydata SttPolydata::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "STT_POLYDATA body requires " << kBodySize
            << " bytes, got " << length;
