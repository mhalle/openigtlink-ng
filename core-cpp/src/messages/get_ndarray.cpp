...
std::vector<std::uint8_t> GetNdarray::pack() const {
...
GetNdarray GetNdarray::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "GET_NDARRAY body requires " << kBodySize
            << " bytes, got " << length;
