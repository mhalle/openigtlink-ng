...
std::vector<std::uint8_t> RtsPosition::pack() const {
...
RtsPosition RtsPosition::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "RTS_POSITION body requires " << kBodySize
            << " bytes, got " << length;
