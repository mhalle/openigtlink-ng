...
std::vector<std::uint8_t> GetPolydata::pack() const {
...
GetPolydata GetPolydata::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "GET_POLYDATA body requires " << kBodySize
            << " bytes, got " << length;
