...
std::vector<std::uint8_t> StpPolydata::pack() const {
...
StpPolydata StpPolydata::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "STP_POLYDATA body requires " << kBodySize
            << " bytes, got " << length;
