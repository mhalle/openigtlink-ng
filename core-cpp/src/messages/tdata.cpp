...
std::vector<std::uint8_t> Tdata::pack() const {
    const std::size_t body_size = (tools.size() * 70);
    ...
    for (std::size_t i = 0; i < tools.size(); ++i) {
        ...
        for (std::size_t i = 0; i < 12; ++i) {
            oigtl::runtime::byte_order::write_be_f32(out.data() + off + i * 4, elem.transform[i]);
        }
        off += 48;
    }
...
Tdata Tdata::unpack(const std::uint8_t* data, std::size_t length) {
    Tdata out;
    std::size_t off = 0;
    // tools
    {
        std::size_t bytes = length - off;
        if (bytes % 70 != 0) { throw oigtl::error::MalformedMessageError("tools: trailing bytes not divisible by element size"); }
        std::size_t count = bytes / 70;
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            off += 1;
            for (std::size_t i = 0; i < 12; ++i) {
                elem.transform[i] = oigtl::runtime::byte_order::read_be_f32(data + off + i * 4);
            }
            off += 48;
        }
    }
