...
std::vector<std::uint8_t> GetPosition::pack() const {
...
GetPosition GetPosition::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "GET_POSITION body requires " << kBodySize
            << " bytes, got " << length;
