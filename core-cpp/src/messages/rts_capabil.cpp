...
std::vector<std::uint8_t> RtsCapabil::pack() const {
...
RtsCapabil RtsCapabil::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "RTS_CAPABIL body requires " << kBodySize
            << " bytes, got " << length;
