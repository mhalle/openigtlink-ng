...
std::vector<std::uint8_t> Point::pack() const {
    const std::size_t body_size = (points.size() * 136);
...
Point Point::unpack(const std::uint8_t* data, std::size_t length) {
    Point out;
    std::size_t off = 0;
    // points
    {
        std::size_t bytes = length - off;
        if (bytes % 136 != 0) { throw oigtl::error::MalformedMessageError("points: trailing bytes not divisible by element size"); }
        std::size_t count = bytes / 136;
