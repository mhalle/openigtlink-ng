...
std::vector<std::uint8_t> SttNdarray::pack() const {
...
SttNdarray SttNdarray::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "STT_NDARRAY body requires " << kBodySize
            << " bytes, got " << length;
