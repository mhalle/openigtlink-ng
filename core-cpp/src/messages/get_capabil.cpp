...
std::vector<std::uint8_t> GetCapabil::pack() const {
...
GetCapabil GetCapabil::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "GET_CAPABIL body requires " << kBodySize
            << " bytes, got " << length;
