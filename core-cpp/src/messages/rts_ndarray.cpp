...
std::vector<std::uint8_t> RtsNdarray::pack() const {
...
RtsNdarray RtsNdarray::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "RTS_NDARRAY body requires " << kBodySize
            << " bytes, got " << length;
