...
std::vector<std::uint8_t> StpNdarray::pack() const {
...
StpNdarray StpNdarray::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "STP_NDARRAY body requires " << kBodySize
            << " bytes, got " << length;
