...
std::vector<std::uint8_t> SttTdata::pack() const {
...
SttTdata SttTdata::unpack(const std::uint8_t* data, std::size_t length) {
    if (length < kBodySize) {
        std::ostringstream oss;
        oss << "STT_TDATA body requires " << kBodySize
            << " bytes, got " << length;
    ...
    {
        constexpr std::size_t n = 32;
        const std::size_t len = oigtl::runtime::ascii::null_padded_length(data + off, n, "coord_name");
