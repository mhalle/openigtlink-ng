...
std::vector<std::uint8_t> RtsCommand::pack() const {
...
RtsCommand RtsCommand::unpack(const std::uint8_t* data, std::size_t length) {
    RtsCommand out;
    std::size_t off = 0;
    // command_id
    ...
    {
        constexpr std::size_t n = 128;
        const std::size_t len = oigtl::runtime::ascii::null_padded_length(data + off, n, "command_name");
