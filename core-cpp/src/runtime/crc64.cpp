// CRC-64 ECMA-182 implementation. See crc64.hpp for context.
//
// The base 256-entry table (kTable below) is identical to the one
// in corpus-tools/.../codec/crc64.py, ported verbatim from upstream
// igtl_util.c. The Python codec rounds-trips all 24 upstream
// fixtures with this exact table; if you change anything here,
// expect oracle parity to fail.
//
// We use a slice-by-8 inner loop: each iteration absorbs 8 input
// bytes into the running CRC via XOR, then reduces them through
// 8 parallel 256-entry tables (kTables[0..7]). Compared to the
// straightforward byte-at-a-time loop this gives roughly a 3-4×
// speedup on multi-KB buffers and zero penalty on short ones —
// the tail is still byte-at-a-time using kTables[0].
//
// All tables are computed by a constexpr factory at compile time,
// so there's no static-init cost and no risk of static-init order
// fiasco. Total ROM footprint: 8 * 256 * 8 = 16 KiB.
...
constexpr std::uint64_t kTable[256] = {
...
// Build kTables[0..7]. Each successive table extends the basic
// step by one more byte position:
//   T_{k+1}[i] = T_0[(T_k[i] >> 56) & 0xFF] ^ (T_k[i] << 8)
// At lookup time, each of the 8 bytes in a chunk is reduced by
// its corresponding table and the eight 64-bit results are XORed
// together — algebraically identical to running the byte-at-a-time
// loop 8 times.
constexpr std::array<std::array<std::uint64_t, 256>, 8> make_tables() {
    ...
    for (std::size_t k = 1; k < 8; ++k) {
        for (std::size_t i = 0; i < 256; ++i) {
            ...
            t[k][i] =
                t[0][static_cast<std::uint8_t>(prev >> 56)] ^ (prev << 8);
        }
    }
    return t;
}
...
std::uint64_t crc64(const std::uint8_t* data,
    ...
    // Slice-by-8 main loop. Each iteration consumes 8 input bytes.
    // The table indexing convention: chunk's least-significant byte
    // (after BE load + XOR with crc) goes through kTables[0]; the
    // most-significant byte goes through kTables[7]. This matches
    // the algebra of running the byte-at-a-time step 8 times.
    while (length >= 8) {
        const std::uint64_t chunk =
            byte_order::read_be_u64(data) ^ crc;
        crc = kTables[7][static_cast<std::uint8_t>(chunk >> 56)]
            ^ kTables[6][static_cast<std::uint8_t>(chunk >> 48)]
            ^ kTables[5][static_cast<std::uint8_t>(chunk >> 40)]
            ^ kTables[4][static_cast<std::uint8_t>(chunk >> 32)]
            ^ kTables[3][static_cast<std::uint8_t>(chunk >> 24)]
            ^ kTables[2][static_cast<std::uint8_t>(chunk >> 16)]
            ^ kTables[1][static_cast<std::uint8_t>(chunk >>  8)]
            ^ kTables[0][static_cast<std::uint8_t>(chunk      )];
        data   += 8;
        length -= 8;
    }
    // Tail: byte-at-a-time, same as the original implementation.
    for (std::size_t i = 0; i < length; ++i) {
        const std::uint8_t index = static_cast<std::uint8_t>(
            data[i] ^ ((crc >> 56) & 0xFFU));
        crc = kTables[0][index] ^ (crc << 8);
    }
    return crc;
}
