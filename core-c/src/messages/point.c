...
int oigtl_point_element_pack(
    ...
    if (cap < OIGTL_POINT_ELEMENT_SIZE)
    ...
    (void)eoff;
    return (int)OIGTL_POINT_ELEMENT_SIZE;
}
...
#define _oigtl_point_HEAD_SIZE 0
...
int oigtl_point_count(const uint8_t *buf, size_t len,
    ...
    if (array_bytes % OIGTL_POINT_ELEMENT_SIZE != 0)
        return OIGTL_ERR_MALFORMED;
    *out_count = array_bytes / OIGTL_POINT_ELEMENT_SIZE;
    return OIGTL_OK;
}
...
int oigtl_point_get(const uint8_t *buf, size_t len,
    ...
    if (buf == NULL || out == NULL) return OIGTL_ERR_INVALID_ARG;
    size_t count;
    int rc = oigtl_point_count(buf, len, &count);
    if (rc != OIGTL_OK) return rc;
    if (idx >= count) return OIGTL_ERR_INVALID_ARG;
    size_t head_off = 0;
    const size_t elem_off =
        head_off + idx * OIGTL_POINT_ELEMENT_SIZE;
    return oigtl_point_element_unpack(
...
int oigtl_point_pack(const oigtl_point_t *msg,
    ...
    const size_t need = off
        + count * OIGTL_POINT_ELEMENT_SIZE;
    if (cap < need) return OIGTL_ERR_SHORT_BUFFER;
    for (size_t i = 0; i < count; ++i) {
        int rc2 = oigtl_point_element_pack(
