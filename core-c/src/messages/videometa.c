...
#include "oigtl_c/messages/videometa.h"
...
int oigtl_videometa_element_pack(
    const oigtl_videometa_element_t *elem,
    uint8_t *buf, size_t cap) {
    if (elem == NULL || buf == NULL) return OIGTL_ERR_INVALID_ARG;
    if (cap < OIGTL_VIDEOMETA_ELEMENT_SIZE)
    ...
    oigtl_write_be_i16(buf + eoff, elem->zoom_level);
    eoff += 2;
    oigtl_write_be_f64(buf + eoff, elem->focal_length);
    eoff += 8;
    ...
    for (size_t i = 0; i < 12; ++i) {
        oigtl_write_be_f32(buf + eoff + i * 4, elem->matrix[i]);
    }
    eoff += 48;
    ...
    eoff += 1;
    (void)eoff;
    return (int)OIGTL_VIDEOMETA_ELEMENT_SIZE;
}

int oigtl_videometa_element_unpack(
    const uint8_t *buf, size_t len,
    oigtl_videometa_element_t *out) {
    if (buf == NULL || out == NULL) return OIGTL_ERR_INVALID_ARG;
    if (len < OIGTL_VIDEOMETA_ELEMENT_SIZE)
        return OIGTL_ERR_SHORT_BUFFER;
    ...
    out->zoom_level = oigtl_read_be_i16(buf + eoff);
    eoff += 2;
    out->focal_length = oigtl_read_be_f64(buf + eoff);
    eoff += 8;
    for (size_t i = 0; i < 3; ++i) {
    ...
    eoff += 6;
    for (size_t i = 0; i < 12; ++i) {
        out->matrix[i] = oigtl_read_be_f32(buf + eoff + i * 4);
    }
    eoff += 48;
...
#define _oigtl_videometa_HEAD_SIZE 0

int oigtl_videometa_count(const uint8_t *buf, size_t len,
                               size_t *out_count) {
    ...
    if (array_bytes % OIGTL_VIDEOMETA_ELEMENT_SIZE != 0)
        return OIGTL_ERR_MALFORMED;
    *out_count = array_bytes / OIGTL_VIDEOMETA_ELEMENT_SIZE;
    return OIGTL_OK;
}

int oigtl_videometa_get(const uint8_t *buf, size_t len,
                             size_t idx,
                             oigtl_videometa_element_t *out) {
    if (buf == NULL || out == NULL) return OIGTL_ERR_INVALID_ARG;
    size_t count;
    int rc = oigtl_videometa_count(buf, len, &count);
    if (rc != OIGTL_OK) return rc;
    if (idx >= count) return OIGTL_ERR_INVALID_ARG;
    size_t head_off = 0;
    const size_t elem_off =
        head_off + idx * OIGTL_VIDEOMETA_ELEMENT_SIZE;
    return oigtl_videometa_element_unpack(
        buf + elem_off, len - elem_off, out);
}

int oigtl_videometa_pack(const oigtl_videometa_t *msg,
                              const oigtl_videometa_element_t *elems,
                              size_t count,
                              uint8_t *buf, size_t cap) {
    ...
    const size_t need = off
        + count * OIGTL_VIDEOMETA_ELEMENT_SIZE;
    if (cap < need) return OIGTL_ERR_SHORT_BUFFER;
    for (size_t i = 0; i < count; ++i) {
        int rc2 = oigtl_videometa_element_pack(
