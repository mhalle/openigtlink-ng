...
#include "oigtl_c/messages/qtrans.h"
...
int oigtl_qtrans_pack(const oigtl_qtrans_t *msg,
    ...
    const int need_i = oigtl_qtrans_packed_size(msg);
    ...
    for (size_t i = 0; i < 4; ++i) {
        oigtl_write_be_f32(buf + off + i * 4, msg->quaternion[i]);
    }
    off += 16;
...
int oigtl_qtrans_unpack(const uint8_t *buf, size_t len,
    ...
    off += 12;
    /* quaternion */
    if (off + 16 > len) return OIGTL_ERR_SHORT_BUFFER;
    for (size_t i = 0; i < 4; ++i) {
        out->quaternion[i] = oigtl_read_be_f32(buf + off + i * 4);
    }
    off += 16;
    (void)off;
    (void)buf;
