...
#include "oigtl_c/messages/capability.h"
...
int oigtl_capability_element_pack(
    ...
    if (cap < OIGTL_CAPABILITY_ELEMENT_SIZE)
        return OIGTL_ERR_SHORT_BUFFER;

    size_t eoff = 0;
    {
        size_t n = strlen(elem->value);
        if (n > 12) return OIGTL_ERR_FIELD_RANGE;
        memcpy(buf + eoff, elem->value, n);
        if (n < 12) memset(buf + eoff + n, 0, 12 - n);
        eoff += 12;
    }
    (void)eoff;
    return (int)OIGTL_CAPABILITY_ELEMENT_SIZE;
}
...
int oigtl_capability_element_unpack(
    ...
    if (len < OIGTL_CAPABILITY_ELEMENT_SIZE)
        return OIGTL_ERR_SHORT_BUFFER;
    ...
    size_t eoff = 0;
    {
        int n = oigtl_null_padded_length(buf + eoff, 12);
        if (n < 0) return n;
        memcpy(out->value, buf + eoff, (size_t)n);
        out->value[n] = '\0';
        eoff += 12;
    }
    (void)eoff;
...
#define _oigtl_capability_HEAD_SIZE 0
...
int oigtl_capability_count(const uint8_t *buf, size_t len,
    ...
    if (array_bytes % OIGTL_CAPABILITY_ELEMENT_SIZE != 0)
        return OIGTL_ERR_MALFORMED;
    *out_count = array_bytes / OIGTL_CAPABILITY_ELEMENT_SIZE;
    return OIGTL_OK;
}
...
int oigtl_capability_get(const uint8_t *buf, size_t len,
    ...
    if (buf == NULL || out == NULL) return OIGTL_ERR_INVALID_ARG;
    size_t count;
    int rc = oigtl_capability_count(buf, len, &count);
    if (rc != OIGTL_OK) return rc;
    if (idx >= count) return OIGTL_ERR_INVALID_ARG;
    size_t head_off = 0;
    const size_t elem_off =
        head_off + idx * OIGTL_CAPABILITY_ELEMENT_SIZE;
    return oigtl_capability_element_unpack(
...
int oigtl_capability_pack(const oigtl_capability_t *msg,
    ...
    const size_t need = off
        + count * OIGTL_CAPABILITY_ELEMENT_SIZE;
    if (cap < need) return OIGTL_ERR_SHORT_BUFFER;
    for (size_t i = 0; i < count; ++i) {
        int rc2 = oigtl_capability_element_pack(
