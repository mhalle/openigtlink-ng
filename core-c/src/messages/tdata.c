...
#include "oigtl_c/messages/tdata.h"
...
int oigtl_tdata_element_pack(
    ...
    if (cap < OIGTL_TDATA_ELEMENT_SIZE)
    ...
    for (size_t i = 0; i < 12; ++i) {
        oigtl_write_be_f32(buf + eoff + i * 4, elem->transform[i]);
    }
    eoff += 48;
    (void)eoff;
    return (int)OIGTL_TDATA_ELEMENT_SIZE;
}
...
int oigtl_tdata_element_unpack(
    ...
    if (len < OIGTL_TDATA_ELEMENT_SIZE)
        return OIGTL_ERR_SHORT_BUFFER;
    ...
    eoff += 1;
    for (size_t i = 0; i < 12; ++i) {
        out->transform[i] = oigtl_read_be_f32(buf + eoff + i * 4);
    }
    eoff += 48;
    (void)eoff;
    (void)buf;
...
#define _oigtl_tdata_HEAD_SIZE 0
...
int oigtl_tdata_count(const uint8_t *buf, size_t len,
    ...
    if (array_bytes % OIGTL_TDATA_ELEMENT_SIZE != 0)
        return OIGTL_ERR_MALFORMED;
    *out_count = array_bytes / OIGTL_TDATA_ELEMENT_SIZE;
    return OIGTL_OK;
}
...
int oigtl_tdata_get(const uint8_t *buf, size_t len,
    ...
    if (buf == NULL || out == NULL) return OIGTL_ERR_INVALID_ARG;
    size_t count;
    int rc = oigtl_tdata_count(buf, len, &count);
    if (rc != OIGTL_OK) return rc;
    if (idx >= count) return OIGTL_ERR_INVALID_ARG;
    size_t head_off = 0;
    const size_t elem_off =
        head_off + idx * OIGTL_TDATA_ELEMENT_SIZE;
    return oigtl_tdata_element_unpack(
...
int oigtl_tdata_pack(const oigtl_tdata_t *msg,
    ...
    const size_t need = off
        + count * OIGTL_TDATA_ELEMENT_SIZE;
    if (cap < need) return OIGTL_ERR_SHORT_BUFFER;
    for (size_t i = 0; i < count; ++i) {
        int rc2 = oigtl_tdata_element_pack(
