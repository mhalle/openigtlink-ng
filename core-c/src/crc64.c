/* CRC-64 ECMA-182 — byte-at-a-time table-driven implementation.
 *
 * Table taken verbatim from corpus-tools/.../codec/crc64.py, which
 * is itself ported from upstream igtl_util.c. All three variants
 * (Python, C++, C) produce identical CRCs for the same input; the
 * oracle fuzzer verifies this every CI run.
 *
 * ROM cost: 2 KiB for the table, ~40 bytes for the loop. Single-
 * table design is deliberate for embedded: the slice-by-8 variant
 * in core-cpp/ is ~4× faster on multi-KB buffers but costs 16 KiB
 * of ROM, which is untenable on an MCU with a 64–128 KiB flash.
 * If a user measures the single-table version as too slow on their
 * target, they can swap in a bigger table without any API change.
 */

#include "oigtl_c/crc64.h"

static const uint64_t k_table[256] = {
...
uint64_t oigtl_crc64_update(uint64_t crc, const uint8_t *data, size_t len) {
    ...
    for (size_t i = 0; i < len; ++i) {
        const uint8_t idx = (uint8_t)((crc >> 56) ^ data[i]);
        crc = (crc << 8) ^ k_table[idx];
    }
    return crc;
}

uint64_t oigtl_crc64(const uint8_t *data, size_t len) {
    return oigtl_crc64_update(UINT64_C(0), data, len);
}
