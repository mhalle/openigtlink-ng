 ...
 *
 * GET_QTRANS — Request the current QTRANS for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_QTRANS_H
#define OIGTL_C_MESSAGES_GET_QTRANS_H
...
/* Fixed body size — every wire-valid GET_QTRANS has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_QTRANS_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_qtrans {
...
int oigtl_get_qtrans_packed_size(const oigtl_get_qtrans_t *msg);
...
int oigtl_get_qtrans_pack(const oigtl_get_qtrans_t *msg,
...
int oigtl_get_qtrans_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_qtrans_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_QTRANS_H */
