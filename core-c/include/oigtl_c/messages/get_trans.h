 ...
 *
 * GET_TRANS — Request the current TRANSFORM for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_TRANS_H
#define OIGTL_C_MESSAGES_GET_TRANS_H
...
/* Fixed body size — every wire-valid GET_TRANS has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_TRANS_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_trans {
...
int oigtl_get_trans_packed_size(const oigtl_get_trans_t *msg);
...
int oigtl_get_trans_pack(const oigtl_get_trans_t *msg,
...
int oigtl_get_trans_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_trans_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_TRANS_H */
