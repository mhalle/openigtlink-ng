 ...
 *
 * STP_TRANS — Stop streaming TRANSFORM messages previously started by STT_TRANS.
 */
#ifndef OIGTL_C_MESSAGES_STP_TRANS_H
#define OIGTL_C_MESSAGES_STP_TRANS_H
...
/* Fixed body size — every wire-valid STP_TRANS has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STP_TRANS_BODY_SIZE ((size_t)0)

typedef struct oigtl_stp_trans {
...
int oigtl_stp_trans_packed_size(const oigtl_stp_trans_t *msg);
...
int oigtl_stp_trans_pack(const oigtl_stp_trans_t *msg,
...
int oigtl_stp_trans_unpack(const uint8_t *buf, size_t len,
                                oigtl_stp_trans_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STP_TRANS_H */
