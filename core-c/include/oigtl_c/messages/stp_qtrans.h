 ...
 *
 * STP_QTRANS — Stop streaming QTRANS messages previously started by STT_QTRANS.
 */
#ifndef OIGTL_C_MESSAGES_STP_QTRANS_H
#define OIGTL_C_MESSAGES_STP_QTRANS_H
...
/* Fixed body size — every wire-valid STP_QTRANS has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STP_QTRANS_BODY_SIZE ((size_t)0)

typedef struct oigtl_stp_qtrans {
...
int oigtl_stp_qtrans_packed_size(const oigtl_stp_qtrans_t *msg);
...
int oigtl_stp_qtrans_pack(const oigtl_stp_qtrans_t *msg,
...
int oigtl_stp_qtrans_unpack(const uint8_t *buf, size_t len,
                                oigtl_stp_qtrans_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STP_QTRANS_H */
