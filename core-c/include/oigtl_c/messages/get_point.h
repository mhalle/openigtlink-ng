 ...
 *
 * GET_POINT — Request the POINT set for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_POINT_H
#define OIGTL_C_MESSAGES_GET_POINT_H
...
/* Fixed body size — every wire-valid GET_POINT has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_POINT_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_point {
...
int oigtl_get_point_packed_size(const oigtl_get_point_t *msg);
...
int oigtl_get_point_pack(const oigtl_get_point_t *msg,
...
int oigtl_get_point_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_point_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_POINT_H */
