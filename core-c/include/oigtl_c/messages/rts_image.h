 ...
 *
 * RTS_IMAGE — Server's return status for a IMAGE query (GET/STT/STP). A single int8: 0 = success, 1 = error.
 */
#ifndef OIGTL_C_MESSAGES_RTS_IMAGE_H
#define OIGTL_C_MESSAGES_RTS_IMAGE_H
...
/* Fixed body size — every wire-valid RTS_IMAGE has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_RTS_IMAGE_BODY_SIZE ((size_t)1)

typedef struct oigtl_rts_image {
    int8_t status;
} oigtl_rts_image_t;
...
int oigtl_rts_image_packed_size(const oigtl_rts_image_t *msg);
...
int oigtl_rts_image_pack(const oigtl_rts_image_t *msg,
...
int oigtl_rts_image_unpack(const uint8_t *buf, size_t len,
                                oigtl_rts_image_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_RTS_IMAGE_H */
