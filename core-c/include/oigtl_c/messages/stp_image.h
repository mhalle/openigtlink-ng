 ...
 *
 * STP_IMAGE — Stop streaming IMAGE messages previously started by STT_IMAGE.
 */
#ifndef OIGTL_C_MESSAGES_STP_IMAGE_H
#define OIGTL_C_MESSAGES_STP_IMAGE_H
...
/* Fixed body size — every wire-valid STP_IMAGE has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STP_IMAGE_BODY_SIZE ((size_t)0)

typedef struct oigtl_stp_image {
...
int oigtl_stp_image_packed_size(const oigtl_stp_image_t *msg);
...
int oigtl_stp_image_pack(const oigtl_stp_image_t *msg,
...
int oigtl_stp_image_unpack(const uint8_t *buf, size_t len,
                                oigtl_stp_image_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STP_IMAGE_H */
