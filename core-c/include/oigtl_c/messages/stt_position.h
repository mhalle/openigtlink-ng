 ...
 *
 * STT_POSITION — Start streaming POSITION messages at the server's default rate.
 */
#ifndef OIGTL_C_MESSAGES_STT_POSITION_H
#define OIGTL_C_MESSAGES_STT_POSITION_H
...
/* Fixed body size — every wire-valid STT_POSITION has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STT_POSITION_BODY_SIZE ((size_t)0)

typedef struct oigtl_stt_position {
...
int oigtl_stt_position_packed_size(const oigtl_stt_position_t *msg);
...
int oigtl_stt_position_pack(const oigtl_stt_position_t *msg,
...
int oigtl_stt_position_unpack(const uint8_t *buf, size_t len,
                                oigtl_stt_position_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STT_POSITION_H */
