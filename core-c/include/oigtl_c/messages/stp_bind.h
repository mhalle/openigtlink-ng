 ...
 *
 * STP_BIND — Stop streaming BIND messages previously started by STT_BIND.
 */
#ifndef OIGTL_C_MESSAGES_STP_BIND_H
#define OIGTL_C_MESSAGES_STP_BIND_H
...
/* Fixed body size — every wire-valid STP_BIND has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STP_BIND_BODY_SIZE ((size_t)0)

typedef struct oigtl_stp_bind {
...
int oigtl_stp_bind_packed_size(const oigtl_stp_bind_t *msg);
...
int oigtl_stp_bind_pack(const oigtl_stp_bind_t *msg,
...
int oigtl_stp_bind_unpack(const uint8_t *buf, size_t len,
                                oigtl_stp_bind_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STP_BIND_H */
