 ...
 *
 * RTS_QTRANS — Server's return status for a QTRANS query (GET/STT/STP). Per Documents/Protocol/query.md, every message type has an RTS_ form for error returns. A single int8: 0 = success, 1 = error.
 */
#ifndef OIGTL_C_MESSAGES_RTS_QTRANS_H
#define OIGTL_C_MESSAGES_RTS_QTRANS_H
...
/* Fixed body size — every wire-valid RTS_QTRANS has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_RTS_QTRANS_BODY_SIZE ((size_t)1)

typedef struct oigtl_rts_qtrans {
    int8_t status;
} oigtl_rts_qtrans_t;
...
int oigtl_rts_qtrans_packed_size(const oigtl_rts_qtrans_t *msg);
...
int oigtl_rts_qtrans_pack(const oigtl_rts_qtrans_t *msg,
...
int oigtl_rts_qtrans_unpack(const uint8_t *buf, size_t len,
                                oigtl_rts_qtrans_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_RTS_QTRANS_H */
