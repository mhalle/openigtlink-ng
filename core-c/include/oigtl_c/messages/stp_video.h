 ...
 *
 * STP_VIDEO — Stop streaming VIDEO frames previously started by STT_VIDEO.
 */
#ifndef OIGTL_C_MESSAGES_STP_VIDEO_H
#define OIGTL_C_MESSAGES_STP_VIDEO_H
...
/* Fixed body size — every wire-valid STP_VIDEO has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STP_VIDEO_BODY_SIZE ((size_t)0)

typedef struct oigtl_stp_video {
...
int oigtl_stp_video_packed_size(const oigtl_stp_video_t *msg);
...
int oigtl_stp_video_pack(const oigtl_stp_video_t *msg,
...
int oigtl_stp_video_unpack(const uint8_t *buf, size_t len,
                                oigtl_stp_video_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STP_VIDEO_H */
