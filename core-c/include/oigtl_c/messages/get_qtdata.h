 ...
 *
 * GET_QTDATA — Request the current QTDATA for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_QTDATA_H
#define OIGTL_C_MESSAGES_GET_QTDATA_H
...
/* Fixed body size — every wire-valid GET_QTDATA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_QTDATA_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_qtdata {
...
int oigtl_get_qtdata_packed_size(const oigtl_get_qtdata_t *msg);
...
int oigtl_get_qtdata_pack(const oigtl_get_qtdata_t *msg,
...
int oigtl_get_qtdata_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_qtdata_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_QTDATA_H */
