 ...
 *
 * STP_NDARRAY — Stop streaming NDARRAY messages previously started by STT_NDARRAY.
 */
#ifndef OIGTL_C_MESSAGES_STP_NDARRAY_H
#define OIGTL_C_MESSAGES_STP_NDARRAY_H
...
/* Fixed body size — every wire-valid STP_NDARRAY has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STP_NDARRAY_BODY_SIZE ((size_t)0)

typedef struct oigtl_stp_ndarray {
...
int oigtl_stp_ndarray_packed_size(const oigtl_stp_ndarray_t *msg);
...
int oigtl_stp_ndarray_pack(const oigtl_stp_ndarray_t *msg,
...
int oigtl_stp_ndarray_unpack(const uint8_t *buf, size_t len,
                                oigtl_stp_ndarray_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STP_NDARRAY_H */
