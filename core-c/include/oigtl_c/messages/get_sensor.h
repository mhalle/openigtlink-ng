/* GENERATED from spec/schemas/get_sensor.json — do not edit.
 *
 * Regenerate with: uv run oigtl-corpus codegen c
 *
 * GET_SENSOR — Request the current SENSOR reading for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_SENSOR_H
#define OIGTL_C_MESSAGES_GET_SENSOR_H
...
/* Fixed body size — every wire-valid GET_SENSOR has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_SENSOR_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_sensor {
...
int oigtl_get_sensor_packed_size(const oigtl_get_sensor_t *msg);
...
int oigtl_get_sensor_pack(const oigtl_get_sensor_t *msg,
...
int oigtl_get_sensor_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_sensor_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_SENSOR_H */
