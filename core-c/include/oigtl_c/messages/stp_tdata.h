 ...
 *
 * STP_TDATA — Stop streaming TDATA messages previously started by STT_TDATA.
 */
#ifndef OIGTL_C_MESSAGES_STP_TDATA_H
#define OIGTL_C_MESSAGES_STP_TDATA_H
...
/* Fixed body size — every wire-valid STP_TDATA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STP_TDATA_BODY_SIZE ((size_t)0)

typedef struct oigtl_stp_tdata {
...
int oigtl_stp_tdata_packed_size(const oigtl_stp_tdata_t *msg);
...
int oigtl_stp_tdata_pack(const oigtl_stp_tdata_t *msg,
...
int oigtl_stp_tdata_unpack(const uint8_t *buf, size_t len,
                                oigtl_stp_tdata_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STP_TDATA_H */
