/* GENERATED from spec/schemas/get_position.json — do not edit.
 *
 * Regenerate with: uv run oigtl-corpus codegen c
 *
 * GET_POSITION — Request the current POSITION for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_POSITION_H
#define OIGTL_C_MESSAGES_GET_POSITION_H
...
/* Fixed body size — every wire-valid GET_POSITION has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_POSITION_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_position {
...
int oigtl_get_position_packed_size(const oigtl_get_position_t *msg);
...
int oigtl_get_position_pack(const oigtl_get_position_t *msg,
...
int oigtl_get_position_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_position_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_POSITION_H */
