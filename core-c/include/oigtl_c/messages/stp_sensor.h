 ...
 *
 * STP_SENSOR — Stop streaming SENSOR messages previously started by STT_SENSOR.
 */
#ifndef OIGTL_C_MESSAGES_STP_SENSOR_H
#define OIGTL_C_MESSAGES_STP_SENSOR_H
...
/* Fixed body size — every wire-valid STP_SENSOR has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STP_SENSOR_BODY_SIZE ((size_t)0)

typedef struct oigtl_stp_sensor {
...
int oigtl_stp_sensor_packed_size(const oigtl_stp_sensor_t *msg);
...
int oigtl_stp_sensor_pack(const oigtl_stp_sensor_t *msg,
...
int oigtl_stp_sensor_unpack(const uint8_t *buf, size_t len,
                                oigtl_stp_sensor_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STP_SENSOR_H */
