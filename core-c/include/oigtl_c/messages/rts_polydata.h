 ...
 *
 * RTS_POLYDATA — Server's return status for a POLYDATA query (GET/STT/STP). A single int8: 0 = success, 1 = error.
 */
#ifndef OIGTL_C_MESSAGES_RTS_POLYDATA_H
#define OIGTL_C_MESSAGES_RTS_POLYDATA_H
...
/* Fixed body size — every wire-valid RTS_POLYDATA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_RTS_POLYDATA_BODY_SIZE ((size_t)1)

typedef struct oigtl_rts_polydata {
    int8_t status;
} oigtl_rts_polydata_t;
...
#endif /* OIGTL_C_MESSAGES_RTS_POLYDATA_H */
