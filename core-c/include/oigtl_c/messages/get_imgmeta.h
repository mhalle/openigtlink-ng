 ...
 *
 * GET_IMGMETA — Request the IMGMETA listing. If the header device name is empty, a listing for all available images is returned.
 */
#ifndef OIGTL_C_MESSAGES_GET_IMGMETA_H
#define OIGTL_C_MESSAGES_GET_IMGMETA_H
...
/* Fixed body size — every wire-valid GET_IMGMETA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_IMGMETA_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_imgmeta {
...
int oigtl_get_imgmeta_packed_size(const oigtl_get_imgmeta_t *msg);
...
int oigtl_get_imgmeta_pack(const oigtl_get_imgmeta_t *msg,
...
int oigtl_get_imgmeta_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_imgmeta_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_IMGMETA_H */
