 ...
 *
 * GET_POLYDATA — Request the current POLYDATA mesh for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_POLYDATA_H
#define OIGTL_C_MESSAGES_GET_POLYDATA_H
...
/* Fixed body size — every wire-valid GET_POLYDATA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_POLYDATA_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_polydata {
...
int oigtl_get_polydata_packed_size(const oigtl_get_polydata_t *msg);
...
int oigtl_get_polydata_pack(const oigtl_get_polydata_t *msg,
...
int oigtl_get_polydata_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_polydata_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_POLYDATA_H */
