 ...
 *
 * GET_STRING — Request the current STRING for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_STRING_H
#define OIGTL_C_MESSAGES_GET_STRING_H
...
/* Fixed body size — every wire-valid GET_STRING has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_STRING_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_string {
...
int oigtl_get_string_packed_size(const oigtl_get_string_t *msg);
...
int oigtl_get_string_pack(const oigtl_get_string_t *msg,
...
int oigtl_get_string_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_string_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_STRING_H */
