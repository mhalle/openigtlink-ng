 ...
 *
 * GET_COLORT — Request the COLORTABLE for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_COLORT_H
#define OIGTL_C_MESSAGES_GET_COLORT_H
...
/* Fixed body size — every wire-valid GET_COLORT has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_COLORT_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_colort {
...
int oigtl_get_colort_packed_size(const oigtl_get_colort_t *msg);
...
int oigtl_get_colort_pack(const oigtl_get_colort_t *msg,
...
int oigtl_get_colort_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_colort_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_COLORT_H */
