 ...
 *
 * RTS_NDARRAY — Server's return status for a NDARRAY query (GET/STT/STP). Per Documents/Protocol/query.md, every message type has an RTS_ form for error returns. A single int8: 0 = success, 1 = error.
 */
#ifndef OIGTL_C_MESSAGES_RTS_NDARRAY_H
#define OIGTL_C_MESSAGES_RTS_NDARRAY_H
...
/* Fixed body size — every wire-valid RTS_NDARRAY has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_RTS_NDARRAY_BODY_SIZE ((size_t)1)

typedef struct oigtl_rts_ndarray {
    int8_t status;
} oigtl_rts_ndarray_t;
...
int oigtl_rts_ndarray_packed_size(const oigtl_rts_ndarray_t *msg);
...
int oigtl_rts_ndarray_pack(const oigtl_rts_ndarray_t *msg,
...
int oigtl_rts_ndarray_unpack(const uint8_t *buf, size_t len,
                                oigtl_rts_ndarray_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_RTS_NDARRAY_H */
