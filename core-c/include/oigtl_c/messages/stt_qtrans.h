 ...
 *
 * STT_QTRANS — Start streaming QTRANS messages at the server's default rate.
 */
#ifndef OIGTL_C_MESSAGES_STT_QTRANS_H
#define OIGTL_C_MESSAGES_STT_QTRANS_H
...
/* Fixed body size — every wire-valid STT_QTRANS has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STT_QTRANS_BODY_SIZE ((size_t)0)

typedef struct oigtl_stt_qtrans {
...
int oigtl_stt_qtrans_packed_size(const oigtl_stt_qtrans_t *msg);
...
int oigtl_stt_qtrans_pack(const oigtl_stt_qtrans_t *msg,
...
int oigtl_stt_qtrans_unpack(const uint8_t *buf, size_t len,
                                oigtl_stt_qtrans_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STT_QTRANS_H */
