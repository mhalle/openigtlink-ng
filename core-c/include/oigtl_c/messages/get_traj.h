 ...
 *
 * GET_TRAJ — Request the TRAJECTORY set for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_TRAJ_H
#define OIGTL_C_MESSAGES_GET_TRAJ_H
...
/* Fixed body size — every wire-valid GET_TRAJ has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_TRAJ_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_traj {
...
int oigtl_get_traj_packed_size(const oigtl_get_traj_t *msg);
...
int oigtl_get_traj_pack(const oigtl_get_traj_t *msg,
...
int oigtl_get_traj_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_traj_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_TRAJ_H */
