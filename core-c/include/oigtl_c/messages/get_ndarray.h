 ...
 *
 * GET_NDARRAY — Request the current NDARRAY for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_NDARRAY_H
#define OIGTL_C_MESSAGES_GET_NDARRAY_H
...
/* Fixed body size — every wire-valid GET_NDARRAY has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_NDARRAY_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_ndarray {
...
int oigtl_get_ndarray_packed_size(const oigtl_get_ndarray_t *msg);
...
int oigtl_get_ndarray_pack(const oigtl_get_ndarray_t *msg,
...
int oigtl_get_ndarray_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_ndarray_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_NDARRAY_H */
