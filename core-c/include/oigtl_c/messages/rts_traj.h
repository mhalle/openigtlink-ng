 ...
 *
 * RTS_TRAJ — Server's return status for a TRAJECTORY query (GET/STT/STP). Per Documents/Protocol/query.md, every message type has an RTS_ form for error returns. A single int8: 0 = success, 1 = error.
 */
#ifndef OIGTL_C_MESSAGES_RTS_TRAJ_H
#define OIGTL_C_MESSAGES_RTS_TRAJ_H
...
/* Fixed body size — every wire-valid RTS_TRAJ has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_RTS_TRAJ_BODY_SIZE ((size_t)1)

typedef struct oigtl_rts_traj {
    int8_t status;
} oigtl_rts_traj_t;
...
int oigtl_rts_traj_packed_size(const oigtl_rts_traj_t *msg);
...
int oigtl_rts_traj_pack(const oigtl_rts_traj_t *msg,
...
int oigtl_rts_traj_unpack(const uint8_t *buf, size_t len,
                                oigtl_rts_traj_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_RTS_TRAJ_H */
