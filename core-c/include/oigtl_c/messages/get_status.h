 ...
 *
 * GET_STATUS — Request the current STATUS for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_STATUS_H
#define OIGTL_C_MESSAGES_GET_STATUS_H
...
/* Fixed body size — every wire-valid GET_STATUS has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_STATUS_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_status {
...
int oigtl_get_status_packed_size(const oigtl_get_status_t *msg);
...
int oigtl_get_status_pack(const oigtl_get_status_t *msg,
...
int oigtl_get_status_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_status_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_STATUS_H */
