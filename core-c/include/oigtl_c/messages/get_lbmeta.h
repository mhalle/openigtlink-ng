 ...
 *
 * GET_LBMETA — Request the LBMETA listing.
 */
#ifndef OIGTL_C_MESSAGES_GET_LBMETA_H
#define OIGTL_C_MESSAGES_GET_LBMETA_H
...
/* Fixed body size — every wire-valid GET_LBMETA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_LBMETA_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_lbmeta {
...
int oigtl_get_lbmeta_packed_size(const oigtl_get_lbmeta_t *msg);
...
int oigtl_get_lbmeta_pack(const oigtl_get_lbmeta_t *msg,
...
int oigtl_get_lbmeta_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_lbmeta_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_LBMETA_H */
