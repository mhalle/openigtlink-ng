 ...
 *
 * RTS_QTDATA — Server's return status for a QTDATA query (GET/STT/STP). A single int8: 0 = success, 1 = error.
 */
#ifndef OIGTL_C_MESSAGES_RTS_QTDATA_H
#define OIGTL_C_MESSAGES_RTS_QTDATA_H
...
/* Fixed body size — every wire-valid RTS_QTDATA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_RTS_QTDATA_BODY_SIZE ((size_t)1)

typedef struct oigtl_rts_qtdata {
    int8_t status;
} oigtl_rts_qtdata_t;
...
int oigtl_rts_qtdata_packed_size(const oigtl_rts_qtdata_t *msg);
...
int oigtl_rts_qtdata_pack(const oigtl_rts_qtdata_t *msg,
...
int oigtl_rts_qtdata_unpack(const uint8_t *buf, size_t len,
                                oigtl_rts_qtdata_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_RTS_QTDATA_H */
