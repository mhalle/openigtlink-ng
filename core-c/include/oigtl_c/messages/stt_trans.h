 ...
 *
 * STT_TRANS — Start streaming TRANSFORM messages at the server's default rate.
 */
#ifndef OIGTL_C_MESSAGES_STT_TRANS_H
#define OIGTL_C_MESSAGES_STT_TRANS_H
...
/* Fixed body size — every wire-valid STT_TRANS has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_STT_TRANS_BODY_SIZE ((size_t)0)

typedef struct oigtl_stt_trans {
...
int oigtl_stt_trans_packed_size(const oigtl_stt_trans_t *msg);
...
int oigtl_stt_trans_pack(const oigtl_stt_trans_t *msg,
...
int oigtl_stt_trans_unpack(const uint8_t *buf, size_t len,
                                oigtl_stt_trans_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_STT_TRANS_H */
