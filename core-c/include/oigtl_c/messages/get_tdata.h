 ...
 *
 * GET_TDATA — Request the current TDATA for the device named in the header.
 */
#ifndef OIGTL_C_MESSAGES_GET_TDATA_H
#define OIGTL_C_MESSAGES_GET_TDATA_H
...
/* Fixed body size — every wire-valid GET_TDATA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_GET_TDATA_BODY_SIZE ((size_t)0)

typedef struct oigtl_get_tdata {
...
int oigtl_get_tdata_packed_size(const oigtl_get_tdata_t *msg);
...
int oigtl_get_tdata_pack(const oigtl_get_tdata_t *msg,
...
int oigtl_get_tdata_unpack(const uint8_t *buf, size_t len,
                                oigtl_get_tdata_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_GET_TDATA_H */
