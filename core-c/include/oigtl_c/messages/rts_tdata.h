/* GENERATED from spec/schemas/rts_tdata.json — do not edit.
 *
 * Regenerate with: uv run oigtl-corpus codegen c
 *
 * RTS_TDATA — Server's return status for a TDATA query (GET/STT/STP). A single int8: 0 = success, 1 = error.
 */
#ifndef OIGTL_C_MESSAGES_RTS_TDATA_H
#define OIGTL_C_MESSAGES_RTS_TDATA_H
...
/* Fixed body size — every wire-valid RTS_TDATA has this
 * many body bytes. Useful for stack-sizing a pack buffer. */
#define OIGTL_RTS_TDATA_BODY_SIZE ((size_t)1)

typedef struct oigtl_rts_tdata {
    int8_t status;
} oigtl_rts_tdata_t;
...
int oigtl_rts_tdata_packed_size(const oigtl_rts_tdata_t *msg);
...
int oigtl_rts_tdata_pack(const oigtl_rts_tdata_t *msg,
...
int oigtl_rts_tdata_unpack(const uint8_t *buf, size_t len,
                                oigtl_rts_tdata_t *out);

#ifdef __cplusplus
}
#endif

#endif /* OIGTL_C_MESSAGES_RTS_TDATA_H */
