...
/**
 * RTS_TDATA — typed TypeScript wire codec.
 */
...
export interface RtsTdataInit {
  status?: number;
}

export class RtsTdata {
  static readonly TYPE_ID = "RTS_TDATA";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsTdataInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsTdata {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_TDATA body must be 1 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `RTS_TDATA unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new RtsTdata({
      status,
    });
    return instance;
  }
