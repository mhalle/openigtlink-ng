...
/**
 * GET_COLORT — typed TypeScript wire codec.
 */
...
export interface GetColortInit {
}

export class GetColort {
  static readonly TYPE_ID = "GET_COLORT";
  static readonly BODY_SIZE = 0;


  constructor(init: GetColortInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetColort {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_COLORT body must be 0 bytes, got ${bytes.length}`,
      );
    }
