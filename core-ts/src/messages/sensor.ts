...
/**
 * SENSOR — typed TypeScript wire codec.
 */
...
export class Sensor {
  static readonly TYPE_ID = "SENSOR";
  ...
  constructor(init: SensorInit = {}) {
    this.larray = init.larray ?? 0;
    this.status = init.status ?? 0;
    this.unit = init.unit ?? 0n;
    this.data = init.data ?? new Float64Array(0);
  }

  static unpack(bytes: Uint8Array): Sensor {
    const view = viewOf(bytes);
    let offset = 0;
    const larray = readU8(view, offset); offset += 1;
    const status = readU8(view, offset); offset += 1;
    const unit = readU64(view, offset); offset += 8;
    const _n_data = Number(larray);
    const data = readF64ArrayBE(view, offset, _n_data); offset += _n_data * 8;
  ...
  pack(): Uint8Array {
    const parts: Uint8Array[] = [];
    {
      ...
      writeU8(viewOf(part), 0, this.larray);
      parts.push(part);
    }
    {
      const part = new Uint8Array(1);
      writeU8(viewOf(part), 0, this.status);
      parts.push(part);
    }
    {
      const part = new Uint8Array(8);
      writeU64(viewOf(part), 0, this.unit);
      parts.push(part);
    }
    {
      const part = new Uint8Array(this.data.length * 8);
      writeF64ArrayBE(viewOf(part), 0, this.data);
      parts.push(part);
    }
    let total = 0;
