...
/**
 * RTS_QTRANS — typed TypeScript wire codec.
 */
...
export interface RtsQtransInit {
  status?: number;
}

export class RtsQtrans {
  static readonly TYPE_ID = "RTS_QTRANS";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsQtransInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsQtrans {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_QTRANS body must be 1 bytes, got ${bytes.length}`,
      );
    }
