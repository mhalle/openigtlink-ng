...
/**
 * RTS_STATUS — typed TypeScript wire codec.
 */
...
export interface RtsStatusInit {
  status?: number;
}

export class RtsStatus {
  static readonly TYPE_ID = "RTS_STATUS";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsStatusInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsStatus {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_STATUS body must be 1 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `RTS_STATUS unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new RtsStatus({
      status,
    });
    return instance;
  }
