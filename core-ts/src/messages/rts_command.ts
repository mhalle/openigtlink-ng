...
/**
 * RTS_COMMAND — typed TypeScript wire codec.
 */
...
export interface RtsCommandInit {
  command_id?: number;
  command_name?: string;
  encoding?: number;
  length?: number;
...
export class RtsCommand {
  static readonly TYPE_ID = "RTS_COMMAND";
  ...
  constructor(init: RtsCommandInit = {}) {
  ...
  static unpack(bytes: Uint8Array): RtsCommand {
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `RTS_COMMAND unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new RtsCommand({
      command_id,
      command_name,
      encoding,
      length,
