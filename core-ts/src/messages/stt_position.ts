...
/**
 * STT_POSITION — typed TypeScript wire codec.
 */
...
export interface SttPositionInit {
}

export class SttPosition {
  static readonly TYPE_ID = "STT_POSITION";
  static readonly BODY_SIZE = 0;


  constructor(init: SttPositionInit = {}) {
  }

  static unpack(bytes: Uint8Array): SttPosition {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STT_POSITION body must be 0 bytes, got ${bytes.length}`,
      );
    }
