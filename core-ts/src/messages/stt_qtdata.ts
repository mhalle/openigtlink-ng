...
/**
 * STT_QTDATA — typed TypeScript wire codec.
 */
...
export interface SttQtdataInit {
  resolution?: number;
  coord_name?: string;
}

export class SttQtdata {
  static readonly TYPE_ID = "STT_QTDATA";
  static readonly BODY_SIZE = 36;

  resolution: number;
  coord_name: string;

  constructor(init: SttQtdataInit = {}) {
    this.resolution = init.resolution ?? 0;
    this.coord_name = init.coord_name ?? "";
  }

  static unpack(bytes: Uint8Array): SttQtdata {
    if (bytes.length !== 36) {
      throw new BodyDecodeError(
        `STT_QTDATA body must be 36 bytes, got ${bytes.length}`,
      );
    }
