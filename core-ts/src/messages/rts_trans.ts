...
/**
 * RTS_TRANS — typed TypeScript wire codec.
 */
...
export interface RtsTransInit {
  status?: number;
}

export class RtsTrans {
  static readonly TYPE_ID = "RTS_TRANS";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsTransInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsTrans {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_TRANS body must be 1 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `RTS_TRANS unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new RtsTrans({
      status,
    });
    return instance;
  }
