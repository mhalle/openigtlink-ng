...
/**
 * GET_POLYDATA — typed TypeScript wire codec.
 */
...
export interface GetPolydataInit {
}

export class GetPolydata {
  static readonly TYPE_ID = "GET_POLYDATA";
  static readonly BODY_SIZE = 0;


  constructor(init: GetPolydataInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetPolydata {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_POLYDATA body must be 0 bytes, got ${bytes.length}`,
      );
    }
