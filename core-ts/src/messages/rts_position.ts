...
/**
 * RTS_POSITION — typed TypeScript wire codec.
 */
...
export interface RtsPositionInit {
  status?: number;
}

export class RtsPosition {
  static readonly TYPE_ID = "RTS_POSITION";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsPositionInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsPosition {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_POSITION body must be 1 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `RTS_POSITION unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new RtsPosition({
      status,
    });
    return instance;
  }
