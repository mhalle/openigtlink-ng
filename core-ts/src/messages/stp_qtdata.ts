...
/**
 * STP_QTDATA — typed TypeScript wire codec.
 */
...
export interface StpQtdataInit {
}

export class StpQtdata {
  static readonly TYPE_ID = "STP_QTDATA";
  static readonly BODY_SIZE = 0;


  constructor(init: StpQtdataInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpQtdata {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_QTDATA body must be 0 bytes, got ${bytes.length}`,
      );
    }
