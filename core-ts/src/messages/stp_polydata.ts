...
/**
 * STP_POLYDATA — typed TypeScript wire codec.
 */
...
export interface StpPolydataInit {
}

export class StpPolydata {
  static readonly TYPE_ID = "STP_POLYDATA";
  static readonly BODY_SIZE = 0;


  constructor(init: StpPolydataInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpPolydata {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_POLYDATA body must be 0 bytes, got ${bytes.length}`,
      );
    }
