...
/**
 * STT_VIDEO — typed TypeScript wire codec.
 */
...
export interface SttVideoInit {
  codec?: string;
  time_interval?: number;
}

export class SttVideo {
  static readonly TYPE_ID = "STT_VIDEO";
  static readonly BODY_SIZE = 8;

  codec: string;
  time_interval: number;

  constructor(init: SttVideoInit = {}) {
    this.codec = init.codec ?? "";
    this.time_interval = init.time_interval ?? 0;
  }

  static unpack(bytes: Uint8Array): SttVideo {
    if (bytes.length !== 8) {
      throw new BodyDecodeError(
        `STT_VIDEO body must be 8 bytes, got ${bytes.length}`,
      );
    }
    const view = viewOf(bytes);
    let offset = 0;
    const codec = _readAsciiRaw(bytes, offset, 4); offset += 4;
    const time_interval = readU32(view, offset); offset += 4;
  ...
  pack(): Uint8Array {
    ...
    {
      const part = new Uint8Array(4);
      writeU32(viewOf(part), 0, this.time_interval);
      parts.push(part);
    }
    let total = 0;
