...
/**
 * STP_IMAGE — typed TypeScript wire codec.
 */
...
export interface StpImageInit {
}

export class StpImage {
  static readonly TYPE_ID = "STP_IMAGE";
  static readonly BODY_SIZE = 0;


  constructor(init: StpImageInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpImage {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_IMAGE body must be 0 bytes, got ${bytes.length}`,
      );
    }
