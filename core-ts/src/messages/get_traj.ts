...
/**
 * GET_TRAJ — typed TypeScript wire codec.
 */
...
export interface GetTrajInit {
}

export class GetTraj {
  static readonly TYPE_ID = "GET_TRAJ";
  static readonly BODY_SIZE = 0;


  constructor(init: GetTrajInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetTraj {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_TRAJ body must be 0 bytes, got ${bytes.length}`,
      );
    }
