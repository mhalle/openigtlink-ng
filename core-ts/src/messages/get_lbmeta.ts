...
/**
 * GET_LBMETA — typed TypeScript wire codec.
 */
...
export interface GetLbmetaInit {
}

export class GetLbmeta {
  static readonly TYPE_ID = "GET_LBMETA";
  static readonly BODY_SIZE = 0;


  constructor(init: GetLbmetaInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetLbmeta {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_LBMETA body must be 0 bytes, got ${bytes.length}`,
      );
    }
