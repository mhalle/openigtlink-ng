...
/**
 * GET_POSITION — typed TypeScript wire codec.
 */
...
export interface GetPositionInit {
}

export class GetPosition {
  static readonly TYPE_ID = "GET_POSITION";
  static readonly BODY_SIZE = 0;


  constructor(init: GetPositionInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetPosition {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_POSITION body must be 0 bytes, got ${bytes.length}`,
      );
    }
