...
/**
 * RTS_SENSOR — typed TypeScript wire codec.
 */
...
export interface RtsSensorInit {
  status?: number;
}

export class RtsSensor {
  static readonly TYPE_ID = "RTS_SENSOR";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsSensorInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsSensor {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_SENSOR body must be 1 bytes, got ${bytes.length}`,
      );
    }
