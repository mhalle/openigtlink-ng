...
/**
 * RTS_TRAJ — typed TypeScript wire codec.
 */
...
export interface RtsTrajInit {
  status?: number;
}

export class RtsTraj {
  static readonly TYPE_ID = "RTS_TRAJ";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsTrajInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsTraj {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_TRAJ body must be 1 bytes, got ${bytes.length}`,
      );
    }
