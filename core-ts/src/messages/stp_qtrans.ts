...
/**
 * STP_QTRANS — typed TypeScript wire codec.
 */
...
export interface StpQtransInit {
}

export class StpQtrans {
  static readonly TYPE_ID = "STP_QTRANS";
  static readonly BODY_SIZE = 0;


  constructor(init: StpQtransInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpQtrans {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_QTRANS body must be 0 bytes, got ${bytes.length}`,
      );
    }
