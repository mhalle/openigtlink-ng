...
/**
 * GET_QTDATA — typed TypeScript wire codec.
 */
...
export interface GetQtdataInit {
}

export class GetQtdata {
  static readonly TYPE_ID = "GET_QTDATA";
  static readonly BODY_SIZE = 0;


  constructor(init: GetQtdataInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetQtdata {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_QTDATA body must be 0 bytes, got ${bytes.length}`,
      );
    }
