...
/**
 * STT_IMAGE — typed TypeScript wire codec.
 */
...
export interface SttImageInit {
}

export class SttImage {
  static readonly TYPE_ID = "STT_IMAGE";
  static readonly BODY_SIZE = 0;


  constructor(init: SttImageInit = {}) {
  }

  static unpack(bytes: Uint8Array): SttImage {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STT_IMAGE body must be 0 bytes, got ${bytes.length}`,
      );
    }
