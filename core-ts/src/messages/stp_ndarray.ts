...
/**
 * STP_NDARRAY — typed TypeScript wire codec.
 */
...
export interface StpNdarrayInit {
}

export class StpNdarray {
  static readonly TYPE_ID = "STP_NDARRAY";
  static readonly BODY_SIZE = 0;


  constructor(init: StpNdarrayInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpNdarray {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_NDARRAY body must be 0 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `STP_NDARRAY unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new StpNdarray({
    });
    return instance;
  }
