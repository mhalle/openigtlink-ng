...
/**
 * GET_IMAGE — typed TypeScript wire codec.
 */
...
export interface GetImageInit {
}

export class GetImage {
  static readonly TYPE_ID = "GET_IMAGE";
  static readonly BODY_SIZE = 0;


  constructor(init: GetImageInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetImage {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_IMAGE body must be 0 bytes, got ${bytes.length}`,
      );
    }
