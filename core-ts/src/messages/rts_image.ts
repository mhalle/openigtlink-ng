...
/**
 * RTS_IMAGE — typed TypeScript wire codec.
 */
...
export interface RtsImageInit {
  status?: number;
}

export class RtsImage {
  static readonly TYPE_ID = "RTS_IMAGE";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsImageInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsImage {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_IMAGE body must be 1 bytes, got ${bytes.length}`,
      );
    }
