...
/**
 * RTS_POINT — typed TypeScript wire codec.
 */
...
export interface RtsPointInit {
  status?: number;
}

export class RtsPoint {
  static readonly TYPE_ID = "RTS_POINT";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsPointInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsPoint {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_POINT body must be 1 bytes, got ${bytes.length}`,
      );
    }
