...
/**
 * RTS_IMGMETA — typed TypeScript wire codec.
 */
...
export interface RtsImgmetaInit {
  status?: number;
}

export class RtsImgmeta {
  static readonly TYPE_ID = "RTS_IMGMETA";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsImgmetaInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsImgmeta {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_IMGMETA body must be 1 bytes, got ${bytes.length}`,
      );
    }
