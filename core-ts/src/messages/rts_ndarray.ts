...
/**
 * RTS_NDARRAY — typed TypeScript wire codec.
 */
...
export interface RtsNdarrayInit {
  status?: number;
}

export class RtsNdarray {
  static readonly TYPE_ID = "RTS_NDARRAY";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsNdarrayInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsNdarray {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_NDARRAY body must be 1 bytes, got ${bytes.length}`,
      );
    }
