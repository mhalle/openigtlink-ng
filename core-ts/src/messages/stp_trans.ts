...
/**
 * STP_TRANS — typed TypeScript wire codec.
 */
...
export interface StpTransInit {
}

export class StpTrans {
  static readonly TYPE_ID = "STP_TRANS";
  static readonly BODY_SIZE = 0;


  constructor(init: StpTransInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpTrans {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_TRANS body must be 0 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `STP_TRANS unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new StpTrans({
    });
    return instance;
  }
