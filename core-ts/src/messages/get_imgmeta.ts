...
/**
 * GET_IMGMETA — typed TypeScript wire codec.
 */
...
export interface GetImgmetaInit {
}

export class GetImgmeta {
  static readonly TYPE_ID = "GET_IMGMETA";
  static readonly BODY_SIZE = 0;


  constructor(init: GetImgmetaInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetImgmeta {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_IMGMETA body must be 0 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `GET_IMGMETA unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new GetImgmeta({
    });
    return instance;
  }
