...
/**
 * GET_TDATA — typed TypeScript wire codec.
 */
...
export interface GetTdataInit {
}

export class GetTdata {
  static readonly TYPE_ID = "GET_TDATA";
  static readonly BODY_SIZE = 0;


  constructor(init: GetTdataInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetTdata {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_TDATA body must be 0 bytes, got ${bytes.length}`,
      );
    }
