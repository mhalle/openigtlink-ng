...
/**
 * GET_QTRANS — typed TypeScript wire codec.
 */
...
export interface GetQtransInit {
}

export class GetQtrans {
  static readonly TYPE_ID = "GET_QTRANS";
  static readonly BODY_SIZE = 0;


  constructor(init: GetQtransInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetQtrans {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_QTRANS body must be 0 bytes, got ${bytes.length}`,
      );
    }
