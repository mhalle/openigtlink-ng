...
/**
 * STT_POLYDATA — typed TypeScript wire codec.
 */
...
export interface SttPolydataInit {
}

export class SttPolydata {
  static readonly TYPE_ID = "STT_POLYDATA";
  static readonly BODY_SIZE = 0;


  constructor(init: SttPolydataInit = {}) {
  }

  static unpack(bytes: Uint8Array): SttPolydata {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STT_POLYDATA body must be 0 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `STT_POLYDATA unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new SttPolydata({
    });
    return instance;
  }
