...
/**
 * IMAGE — typed TypeScript wire codec.
 */
...
export interface ImageMessageInit {
...
export class ImageMessage {
  ...
  constructor(init: ImageMessageInit = {}) {
    this.header_version = init.header_version ?? 0;
    this.num_components = init.num_components ?? 0;
    ...
    this.pixels = init.pixels ?? new Uint8Array(0);
  }

  static unpack(bytes: Uint8Array): ImageMessage {
    const view = viewOf(bytes);
    let offset = 0;
    const header_version = readU16(view, offset); offset += 2;
    const num_components = readU8(view, offset); offset += 1;
    ...
    const pixels = new Uint8Array(bytes.subarray(offset, bytes.length)); offset = bytes.length;
  ...
  pack(): Uint8Array {
    ...
    }
    {
      ...
      writeU8(viewOf(part), 0, this.num_components);
      parts.push(part);
    }
    {
      const part = new Uint8Array(1);
    ...
    parts.push(new Uint8Array(this.pixels));
    let total = 0;
