...
/**
 * STP_VIDEO — typed TypeScript wire codec.
 */
...
export interface StpVideoInit {
}

export class StpVideo {
  static readonly TYPE_ID = "STP_VIDEO";
  static readonly BODY_SIZE = 0;


  constructor(init: StpVideoInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpVideo {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_VIDEO body must be 0 bytes, got ${bytes.length}`,
      );
    }
