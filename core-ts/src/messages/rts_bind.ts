...
/**
 * RTS_BIND — typed TypeScript wire codec.
 */
...
export interface RtsBindInit {
  status?: number;
}

export class RtsBind {
  static readonly TYPE_ID = "RTS_BIND";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsBindInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsBind {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_BIND body must be 1 bytes, got ${bytes.length}`,
      );
    }
