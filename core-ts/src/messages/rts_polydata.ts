...
/**
 * RTS_POLYDATA — typed TypeScript wire codec.
 */
...
export interface RtsPolydataInit {
  status?: number;
}

export class RtsPolydata {
  static readonly TYPE_ID = "RTS_POLYDATA";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsPolydataInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsPolydata {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_POLYDATA body must be 1 bytes, got ${bytes.length}`,
      );
    }
