...
/**
 * GET_CAPABIL — typed TypeScript wire codec.
 */
...
export interface GetCapabilInit {
}

export class GetCapabil {
  static readonly TYPE_ID = "GET_CAPABIL";
  static readonly BODY_SIZE = 0;


  constructor(init: GetCapabilInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetCapabil {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_CAPABIL body must be 0 bytes, got ${bytes.length}`,
      );
    }
