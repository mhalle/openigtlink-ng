...
/**
 * GET_STRING — typed TypeScript wire codec.
 */
...
export interface GetStringInit {
}

export class GetString {
  static readonly TYPE_ID = "GET_STRING";
  static readonly BODY_SIZE = 0;


  constructor(init: GetStringInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetString {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_STRING body must be 0 bytes, got ${bytes.length}`,
      );
    }
