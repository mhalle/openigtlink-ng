...
/**
 * STT_TRANS — typed TypeScript wire codec.
 */
...
export interface SttTransInit {
}

export class SttTrans {
  static readonly TYPE_ID = "STT_TRANS";
  static readonly BODY_SIZE = 0;


  constructor(init: SttTransInit = {}) {
  }

  static unpack(bytes: Uint8Array): SttTrans {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STT_TRANS body must be 0 bytes, got ${bytes.length}`,
      );
    }
