...
/**
 * GET_BIND — typed TypeScript wire codec.
 */
...
export class GetBind {
  static readonly TYPE_ID = "GET_BIND";
  ...
  static unpack(bytes: Uint8Array): GetBind {
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `GET_BIND unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new GetBind({
      ncmessages,
      type_ids,
      nametable_size,
      name_table,
