...
/**
 * RTS_QTDATA — typed TypeScript wire codec.
 */
...
export interface RtsQtdataInit {
  status?: number;
}

export class RtsQtdata {
  static readonly TYPE_ID = "RTS_QTDATA";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsQtdataInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsQtdata {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_QTDATA body must be 1 bytes, got ${bytes.length}`,
      );
    }
