...
/**
 * GET_SENSOR — typed TypeScript wire codec.
 */
...
export interface GetSensorInit {
}

export class GetSensor {
  static readonly TYPE_ID = "GET_SENSOR";
  static readonly BODY_SIZE = 0;


  constructor(init: GetSensorInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetSensor {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_SENSOR body must be 0 bytes, got ${bytes.length}`,
      );
    }
