...
/**
 * RTS_CAPABIL — typed TypeScript wire codec.
 */
...
export interface RtsCapabilInit {
  status?: number;
}

export class RtsCapabil {
  static readonly TYPE_ID = "RTS_CAPABIL";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsCapabilInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsCapabil {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_CAPABIL body must be 1 bytes, got ${bytes.length}`,
      );
    }
