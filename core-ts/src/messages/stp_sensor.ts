...
/**
 * STP_SENSOR — typed TypeScript wire codec.
 */
...
export interface StpSensorInit {
}

export class StpSensor {
  static readonly TYPE_ID = "STP_SENSOR";
  static readonly BODY_SIZE = 0;


  constructor(init: StpSensorInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpSensor {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_SENSOR body must be 0 bytes, got ${bytes.length}`,
      );
    }
