...
/**
 * STP_POSITION — typed TypeScript wire codec.
 */
...
export interface StpPositionInit {
}

export class StpPosition {
  static readonly TYPE_ID = "STP_POSITION";
  static readonly BODY_SIZE = 0;


  constructor(init: StpPositionInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpPosition {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_POSITION body must be 0 bytes, got ${bytes.length}`,
      );
    }
    ...
    if (offset !== bytes.length) {
      throw new BodyDecodeError(
        `STP_POSITION unpack consumed ${offset}/${bytes.length} bytes`,
      );
    }
    const instance = new StpPosition({
    });
    return instance;
  }
