...
/**
 * GET_VMETA — typed TypeScript wire codec.
 */
...
export interface GetVmetaInit {
}

export class GetVmeta {
  static readonly TYPE_ID = "GET_VMETA";
  static readonly BODY_SIZE = 0;


  constructor(init: GetVmetaInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetVmeta {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_VMETA body must be 0 bytes, got ${bytes.length}`,
      );
    }
