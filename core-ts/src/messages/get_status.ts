...
/**
 * GET_STATUS — typed TypeScript wire codec.
 */
...
export interface GetStatusInit {
}

export class GetStatus {
  static readonly TYPE_ID = "GET_STATUS";
  static readonly BODY_SIZE = 0;


  constructor(init: GetStatusInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetStatus {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_STATUS body must be 0 bytes, got ${bytes.length}`,
      );
    }
