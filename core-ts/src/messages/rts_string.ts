...
/**
 * RTS_STRING — typed TypeScript wire codec.
 */
...
export interface RtsStringInit {
  status?: number;
}

export class RtsString {
  static readonly TYPE_ID = "RTS_STRING";
  static readonly BODY_SIZE = 1;

  status: number;

  constructor(init: RtsStringInit = {}) {
    this.status = init.status ?? 0;
  }

  static unpack(bytes: Uint8Array): RtsString {
    if (bytes.length !== 1) {
      throw new BodyDecodeError(
        `RTS_STRING body must be 1 bytes, got ${bytes.length}`,
      );
    }
