...
/**
 * STP_BIND — typed TypeScript wire codec.
 */
...
export interface StpBindInit {
}

export class StpBind {
  static readonly TYPE_ID = "STP_BIND";
  static readonly BODY_SIZE = 0;


  constructor(init: StpBindInit = {}) {
  }

  static unpack(bytes: Uint8Array): StpBind {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STP_BIND body must be 0 bytes, got ${bytes.length}`,
      );
    }
