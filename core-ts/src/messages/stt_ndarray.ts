...
/**
 * STT_NDARRAY — typed TypeScript wire codec.
 */
...
export interface SttNdarrayInit {
}

export class SttNdarray {
  static readonly TYPE_ID = "STT_NDARRAY";
  static readonly BODY_SIZE = 0;


  constructor(init: SttNdarrayInit = {}) {
  }

  static unpack(bytes: Uint8Array): SttNdarray {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `STT_NDARRAY body must be 0 bytes, got ${bytes.length}`,
      );
    }
