...
/**
 * GET_POINT — typed TypeScript wire codec.
 */
...
export interface GetPointInit {
}

export class GetPoint {
  static readonly TYPE_ID = "GET_POINT";
  static readonly BODY_SIZE = 0;


  constructor(init: GetPointInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetPoint {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_POINT body must be 0 bytes, got ${bytes.length}`,
      );
    }
