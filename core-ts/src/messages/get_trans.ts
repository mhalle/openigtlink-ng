...
/**
 * GET_TRANS — typed TypeScript wire codec.
 */
...
export interface GetTransInit {
}

export class GetTrans {
  static readonly TYPE_ID = "GET_TRANS";
  static readonly BODY_SIZE = 0;


  constructor(init: GetTransInit = {}) {
  }

  static unpack(bytes: Uint8Array): GetTrans {
    if (bytes.length !== 0) {
      throw new BodyDecodeError(
        `GET_TRANS body must be 0 bytes, got ${bytes.length}`,
      );
    }
