...
The lookup table is extracted verbatim from ``igtl_util.c`` (pinned SHA
94244fe). The algorithm is: ``crc = table[byte ^ (crc >> 56)] ^ (crc << 8)``,
iterated over each byte of the input, with the initial CRC value passed in
(typically 0 for the first call, then chained for multi-region CRC as used
by IMAGE, POLYDATA, etc.).
"""
